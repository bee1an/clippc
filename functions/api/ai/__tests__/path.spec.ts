...
describe('functions/api/ai/[[path]]', () => {
  afterEach(() => {
    vi.restoreAllMocks()
  })
  ...
  it('rejects private upstream base url in byok mode', async () => {
    const response = await onRequest({
      request: new Request('https://clippc.com/api/ai/chat/completions', {
        method: 'POST',
        headers: {
  ...
  it('uses byok upstream header in preference to managed upstream', async () => {
    ...
    const response = await onRequest({
      request: new Request('https://clippc.com/api/ai/chat/completions?stream=true', {
        method: 'POST',
        headers: {
          'x-clippc-key-source': 'byok',
  ...
  it('keeps managed upstream host when path looks like absolute url', async () => {
    ...
    const response = await onRequest({
      request: new Request('https://clippc.com/api/ai/https://attacker.example/v1/chat/completions?stream=true', {
        method: 'POST',
        headers: {
  ...
  it('allows managed upstream over local http for self-hosted models', async () => {
    const fetchMock = vi.fn(async () => new Response('ok', { status: 200 }))
    vi.stubGlobal('fetch', fetchMock)
    ...
    const response = await onRequest({
      ...
      env: {
        AI_MANAGED_UPSTREAM: 'http://127.0.0.1:11434',
        AI_MANAGED_API_KEY: 'managed-secret',
      },
      params: {
        path: 'chat/completions',
    ...
    const [requestUrl, requestInit] = fetchMock.mock.calls[0]
    expect(requestUrl).toBe('http://127.0.0.1:11434/v1/chat/completions?stream=true')
    expect(requestInit.headers.get('authorization')).toBe('Bearer managed-secret')
  })
