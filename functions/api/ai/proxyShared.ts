...
export function normalizeUpstreamBase(
  ...
  const httpAllowed = protocol === 'http:' && (allowHttp || httpLocalhostAllowed)
  ...
  if (!allowPrivateHosts && isLocalHostname(hostname) && !httpLocalhostAllowed) {
    return {
      ok: false,
  ...
  if (!allowPrivateHosts && (isPrivateIpv4(hostname) || isPrivateIpv6(hostname))) {
    return {
      ok: false,
