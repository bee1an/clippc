...
const PROXY_BASE_PATH = '/api/ai'
const MANAGED_PLACEHOLDER_API_KEY = 'clippc-managed-placeholder'

type OpenAiCompatibleClientSettings = {
...
function resolveAbsoluteUrl(input: string): string {
  ...
  // Browser runtime: allow relative base URL like "/api/ai".
...
export function resolveOpenAiCompatibleBaseUrl(baseUrl: string): string {
  return stripChatCompletionsSuffix(resolveAbsoluteUrl(baseUrl))
}

export function resolveOpenAiCompatibleChatUrl(baseUrl: string): string {
  return `${resolveOpenAiCompatibleBaseUrl(baseUrl)}/chat/completions`
}
...
export function resolveProxyUpstreamBase(baseUrl: string): string | null {
  ...
  const normalizedBaseUrl = resolveOpenAiCompatibleBaseUrl(baseUrl).trim()
...
function resolveClientApiKey(settings: OpenAiCompatibleClientSettings): string {
  if (settings.apiKeySource === 'byok')
    return settings.apiKey
  return MANAGED_PLACEHOLDER_API_KEY
}

function resolveClientHeaders(settings: OpenAiCompatibleClientSettings): Record<string, string | null> | undefined {
...
function createOpenAiClient(settings: OpenAiCompatibleClientSettings): OpenAI {
...
export const openAiCompatibleClient: ChatProviderClient = {
...
export async function streamChatCompletion(
  ...
  await openAiCompatibleClient.streamChatCompletion(settings, messages, handlers)
}
...
export async function createChatCompletion(
  ...
  return openAiCompatibleClient.createChatCompletion(settings, messages, options)
}
