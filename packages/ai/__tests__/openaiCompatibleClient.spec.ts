...
} from '../src/providers/openaiCompatibleClient'
...
describe('resolveOpenAiCompatibleChatUrl', () => {
  it('keeps /v1 as base url', () => {
    expect(resolveOpenAiCompatibleBaseUrl('https://host/v1')).toBe('https://host/v1')
  })

  it('strips /chat/completions suffix to base url', () => {
    expect(resolveOpenAiCompatibleBaseUrl('https://host/v1/chat/completions')).toBe('https://host/v1')
  })

  it('keeps plain base url unchanged', () => {
    expect(resolveOpenAiCompatibleBaseUrl('https://host/api')).toBe('https://host/api')
  })

  it('keeps /chat/completions unchanged', () => {
    expect(resolveOpenAiCompatibleChatUrl('https://host/v1/chat/completions')).toBe('https://host/v1/chat/completions')
  })

  it('appends /chat/completions when base url ends with /v1', () => {
    expect(resolveOpenAiCompatibleChatUrl('https://host/v1')).toBe('https://host/v1/chat/completions')
  })

  it('appends /chat/completions for plain base url', () => {
    expect(resolveOpenAiCompatibleChatUrl('https://host/api')).toBe('https://host/api/chat/completions')
  })

  it('returns null upstream for internal proxy path', () => {
    expect(resolveProxyUpstreamBase('/api/ai')).toBeNull()
  })
